...
module tb_{{DUT_MODULE}};
    ...
    // Replace with DUT input/output signal declarations.
    // Example:
    // reg  [15:0] in_data;
    // reg         in_valid;
    // wire [15:0] out_data;
    ...
    {{DUT_MODULE}} dut (
        .clk   (clk),
        .rst_n (rst_n)
        // Replace with remaining DUT port connections.
    );

    initial begin
        // Replace with input signal initialization.
        wait (rst_n === 1'b1);
        repeat ({{STIMULUS_CYCLES}}) begin
            @(posedge clk);
            // Replace with stimulus driving logic.
        end
